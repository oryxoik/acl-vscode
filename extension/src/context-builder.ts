...
import { docUriToTokensMap } from "./token-mapper";
import { FunctionDefinition, TypeDefinition, createFunctionDefinition, createTypeDefinition, tryAddVariableToDefinition } from "./parser/definitions";
import { TokenType } from "./lexer/TokenType";
import types from "./builtin-types";

export const typeDefinitions: Map<string, TypeDefinition[]> = new Map<string, TypeDefinition[]>([["built-in", types]]);
export var allTypeDefinitions: TypeDefinition[] = [...types];

export function updateContextForDocument(uri: vscode.Uri) {
    const tokens = docUriToTokensMap.get(uri);
    ...
    for (let i = 0; i < tokens.length; i++) {
        ...
        if (token.type === TokenType.Function && match(i, 1, TokenType.Identifier)) {
            const idToken = tokens[i + 1];
            currentFunc = createFunctionDefinition(token, idToken);
            if (i + 2 < tokens.length && tokens[i + 2].type === TokenType.LeftParen) {
                let next = i + 3;
                while (next < tokens.length && tokens[next].type === TokenType.Identifier) {
        ...
        if (token.type === TokenType.Identifier && match(i, 1, TokenType.Dot) && match(i, 2, TokenType.Identifier)) {
            if (currentDef != null) {
        ...
        if (token.type === TokenType.Identifier && match(i, 1, TokenType.LeftParen) && i - 1 >= 0 && tokens[i - 1].type !== TokenType.Function) {
            if (currentDef != null) {
        ...
        if (token.type === TokenType.Identifier && match(i, 1, TokenType.Equal)) {
            if (currentFunc != null) {
                if (!currentFunc.variables.has(token.lexeme)) {
                    if (i - 1 >= 0 && tokens[i - 1].type === TokenType.Dot && i - 2 >= 0 && tokens[i - 2].type === TokenType.Self) {
    ...
    function match(i: number, offset: number, ...tokenTypes: TokenType[]): boolean {
